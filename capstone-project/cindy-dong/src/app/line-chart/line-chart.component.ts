...
export class LineChartComponent implements OnInit {
    ...
    createChart(filteredData: YearsData[] = this.yearsData, x?: d3.ScaleTime<number, number>) {
        ...
        const selectedCounties = allGroup.filter((county) => filteredData.some((d) => d.COUNTY === county));
        ...
        if (selectedCounties.length > 0) {
            ...
            legend.append('div').style('width', '18px').style('height', '18px').style('background-color', color);
    ...
    setupFilters() {
        ...
        selectYear.on('change', () => {
            ...
            if (startYear && endYear) {
                const x = d3.scaleTime().domain([startYear, endYear]).range([0, this.width]);
